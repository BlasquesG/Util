...
get_NASA_features <- function(startday, endday,
  ...
  all_parameters <- c(
    "TS", "T2M", "T2MDEW", "T2MWET", "TS_MAX", "T2M_MAX", 
    ...
    "WS2M","TS_MIN", "GWETTOP", "GWETPROF", 
    "GWETROOT", "WS2M_MAX", "WS2M_MIN", "WS2M_RANGE", "PRECTOTCORR", "ALLSKY_SFC_LW_DWN"
  )
  ...
  if (length(startday) > 1 || length(latitudes) > 1) {
    ...
    results <- lapply(seq_along(latitudes), function(i) {
      
      date_range_i <- paste(startday[i], endday[i], sep = "_")
      ...
      result <- get_NASA_features(
        ...
        TbaseInf   = TbaseInf,
        TbaseSup   = TbaseSup,
        env        = if (is.null(env)) NULL else env[i],  
        save_csv   = FALSE,
        DoMean     = DoMean,
      ...
      if (!is.null(result)) {
        result <- result %>% mutate(DATE_RANGE = date_range_i)
      }
    ...
    if (save_csv) {
      tryCatch({
        ...
        message("Error saving the file: ", e$message)
      })
    }
  ...
  ## GD
  combined_all_points_data <- combined_all_points_data %>% mutate(
    GD = case_when(
      ...
      T2M_MAX > TbaseSup & TbaseSup > T2M_MIN & T2M_MIN > TbaseInf ~ 
        (2 * (T2M_MAX - T2M_MIN) * (T2M_MIN - TbaseInf) + (T2M_MAX - T2M_MIN)^2 - (T2M_MAX - TbaseSup)) /
        (2 * (T2M_MAX - T2M_MIN)),
      T2M_MAX > TbaseSup & TbaseSup > TbaseInf & TbaseInf > T2M_MIN ~ 
        0.5 * ((T2M_MAX - TbaseInf)^2 - (T2M_MAX - TbaseSup)^2) / (T2M_MAX - T2M_MIN),
      TRUE ~ 0
    )
  )
  ...
  combined_all_points_data <- combined_all_points_data %>%
    mutate(
      VPD = (
        ((0.61078 * exp((17.27 * T2M_MIN) / (T2M_MIN + 237.3))) +
  ...
  colnames(combined_all_points_data) <- sapply(
    colnames(combined_all_points_data),
    function(x) ifelse(x %in% names(param_names), param_names[[x]], x)
  )
  ...
  combined_all_points_data <- combined_all_points_data %>%
    mutate(env = if (!is.null(env)) env else NA_character_) %>%  
    select(env, everything())
  

  combined_all_points_data <- combined_all_points_data %>%
    mutate(DATE_RANGE = paste(startday, endday, sep = "_")) %>%
    select(DATE_RANGE, everything())
